...
User messages (e.g., *"I need food for 5 people, 2 are veg, budget is 2000"*) are fed into **Gemini 3.5 Flash Lite**.
...
The solver's verified output is sent to Gemini 3.5 Flash Lite (`temperature=0.3`) with a strict 7-rule system prompt. The LLM generates a brief, warm 2-3 sentence summary that references ONLY the verified dishes, quantities, and prices. It cannot invent or alter any data.
...
The backend exposes chat, authentication, restaurant/menu, and admin endpoints. Full interactive documentation is auto-generated by FastAPI at `http://localhost:8000/docs` locally or `/docs` on the deployed backend.

Implemented routes include `POST /api/chat`, `GET /api/chat/active`, restaurant/menu GET routes, customer and admin auth routes, `GET /api/admin/metrics`, and `GET /api/admin/conversations`.
